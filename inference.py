...
def face_detect(images, results_file='last_detected_face.pkl'):
    # Guard against stale cache: always re-detect unless we have a valid cache
    # for the CURRENT video (same frame count AND same face path).
    _cache_valid = False
    _cached_frames = 0
    if os.path.exists(results_file):
        try:
            ...
            # Cache format: (face_path, detection_results_list)
            if isinstance(_cached, tuple) and len(_cached) == 2:
                _cached_face_path, _cached_data = _cached
                _cached_frames = len(_cached_data)
                if _cached_frames == len(images) and _cached_face_path == args.face:
                    print(f'Using face detection data from cache ({_cached_frames} frames, path={os.path.basename(_cached_face_path)})')
                    _cache_valid = True
                else:
                    print(f'[WARN] Face cache mismatch: cache frames={_cached_frames}, current frames={len(images)}, '
                          f'cache path={os.path.basename(_cached_face_path) if _cached_face_path else "N/A"}, '
                          f'current path={os.path.basename(args.face)}. Re-detecting.')
            else:
                # Old format cache (just results list) — invalidate
                print('[WARN] Stale face cache format. Re-detecting.')
        except Exception as e:
            print(f'[WARN] Could not read face cache ({e}). Re-detecting.')

    if _cache_valid:
        _, _cached_data = None, None
        with open(results_file, 'rb') as f:
            _cached = pickle.load(f)
        return _cached[1]
    ...
    # Save results with face path to detect cache changes
    with open(results_file, 'wb') as f:
        pickle.dump((args.face, results), f)
...
def datagen(frames, mels):
    ...
    # Safeguard: face_det_results must cover all frames we will iterate over.
    # If cached data has a different length than current frames (e.g. different
    # video lengths across runs), rebuild detection results to match current length.
    if len(face_det_results) < len(frames):
        print(f'[WARN] Cached face detections ({len(face_det_results)}) shorter than '
              f'frames ({len(frames)}). Re-detecting faces...')
        face_det_results = face_detect(frames)

    for i, m in enumerate(mels):
        idx = 0 if args.static else i % len(frames)
        # Clamp idx so we never exceed the length of face_det_results.
        safe_idx = min(idx, len(face_det_results) - 1)
        frame_to_save = frames[idx].copy()
        face, coords = face_det_results[safe_idx].copy()
