...
use_fixed_box = config.getboolean('BOX', 'use_fixed_box')
box_top = config.getint('BOX', 'box_top')
box_bottom = config.getint('BOX', 'box_bottom')
box_left = config.getint('BOX', 'box_left')
box_right = config.getint('BOX', 'box_right')
crop_top = config.getint('CROP', 'crop_top')
crop_bottom = config.getint('CROP', 'crop_bottom')
crop_left = config.getint('CROP', 'crop_left')
crop_right = config.getint('CROP', 'crop_right')
...
if include_settings_in_suffix:
  ...
  if use_fixed_box:
    output_suffix = f'{output_suffix}_box-{box_top}-{box_bottom}-{box_left}-{box_right}'
  if crop_top != 0 or crop_bottom != -1 or crop_left != 0 or crop_right != -1:
    output_suffix = f'{output_suffix}_crop-{crop_top}-{crop_bottom}-{crop_left}-{crop_right}'
  if quality != 'fast':
...
while True:
  ...
  cmd = [
      ...
      "--box", str(box_top), str(box_bottom), str(box_left), str(box_right),
      "--crop", str(crop_top), str(crop_bottom), str(crop_left), str(crop_right),
